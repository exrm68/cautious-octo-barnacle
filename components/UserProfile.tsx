...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'config'), s => {
      ...
      setCs({
        ...
        referralBotUsername: (d.referralBotUsername || d.botUsername || botUsername || '').replace('@','').trim(),
    ...
    (async () => {
      ...
      try {
        ...
        if (!snap.exists()) {
          ...
          // ✅ Fix: ensure takaBalance exists for old users
          ...
          // ✅ Always sync latest Telegram photo & name
  ...
  if (loading) return (
    <div className="fixed inset-0 z-50 bg-[#09090c] flex items-center justify-center">
          ...
          <motion.div key="main" variants={pv} initial="initial" animate="animate" exit="exit"
            ...
            <div className="relative">
              <div className="absolute inset-0 overflow-hidden" style={{height:260}}>
                {userData?.photo
                  ? <img src={userData.photo} className="w-full h-full object-cover scale-110"
                      style={{filter:'blur(50px) brightness(0.22) saturate(0.5)'}} onError={()=>{}} />
              ...
              <div className="relative z-10 pt-14 pb-5 px-5" style={{minHeight:260}}>
                ...
                <div className="flex flex-col items-center text-center">
                  <div className="relative mb-3">
                    <TgAvatar photo={userData?.photo} name={userData?.name} size={82} />
                  ...
                  <h1 className="text-2xl font-black text-white tracking-tight">{userData?.name || '...'}</h1>
                  <div className="flex items-center gap-2 mt-1 flex-wrap justify-center">
                    {userData?.username && <span className="text-zinc-400 text-sm">@{userData.username}</span>}
                ...
                <div className="relative z-10 p-4">
                  ...
                  <div className="bg-black/30 border border-white/10 rounded-[12px] flex items-center px-3 py-2.5 mb-3 gap-2 min-w-0">
                    <span className="flex-1 text-white/80 font-mono text-[11px] truncate">
                      {getLink() || (userData?.referralCode ? `ref_${userData.referralCode}` : '...')}
                    </span>
          ...
          <motion.div key="convert" variants={pv} initial="initial" animate="animate" exit="exit" className="flex-1 overflow-y-auto no-scrollbar flex flex-col">
            ...
            <div className="p-5 flex-1 flex flex-col gap-5">
              ...
              <div className="bg-[#1a1a1d] border border-white/5 rounded-[20px] p-4 focus-within:border-amber-500/30 transition-colors">
                <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-2">অথবা নিজে লেখো ({cs.coinRate.toLocaleString()} এর গুণিতক)</p>
